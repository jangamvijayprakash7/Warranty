...
export default function DashboardPage() {
  const stats = [
    { label: "Total Items", value: 12, icon: Clock, color: "bg-indigo-50 text-indigo-600" },
    { label: "Active", value: 8, icon: CheckCircle, color: "bg-green-50 text-green-600" },
    { label: "Expiring", value: 3, icon: Clock, color: "bg-orange-50 text-orange-600" },
    { label: "Expired", value: 1, icon: XCircle, color: "bg-red-50 text-red-600" },
  ];

  const warranties = [
    { name: "MacBook Pro 14\"", expiry: "Expiring in 1 month", status: "Expiring" },
    { name: "Sony WH‑1000XM5 Headphones", expiry: "Expiring on Nov 21, 2025", status: "Active" },
    { name: "iPhone 15 Pro Max", expiry: "Expiring on Mar 21, 2026", status: "Active" },
    { name: "Samsung 4K Smart TV", expiry: "Expiring in 1 week", status: "Expiring" },
  ];
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-2xl bg-blue-50 p-6">
            ...
            <p className="mt-2 text-xs text-slate-500">Last synced: 22/09/2025, 02:45</p>

            <div className="mt-4 flex gap-2">
              <button className="flex-1 rounded-xl bg-indigo-600 py-2 text-sm text-white">
                Sync Warranties
              </button>
              <button className="flex-1 rounded-xl border py-2 text-sm">Disconnect</button>
            </div>
          </div>
        </div>
      </div>
